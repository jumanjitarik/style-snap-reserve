...
function ReviewSection({ shopId, userId, reviews }: { shopId: string; userId: string | null; reviews: { id: string; rating: number; comment: string | null; created_at: string; user_id: string; author_name?: string }[] }) {
  const qc = useQueryClient();
        ...
        {reviews.map((r) => (
          <div key={r.id} className="rounded-xl border border-border bg-card p-3">
            <div className="flex items-center justify-between gap-2">
              <p className="text-sm font-semibold">{r.author_name ?? "Kullanıcı"}</p>
              <p className="text-[10px] text-muted-foreground">{new Date(r.created_at).toLocaleDateString("tr-TR", { day: "numeric", month: "long", year: "numeric" })}</p>
            </div>
...
function WaitlistWidget({ shopId, userId }: { shopId: string; userId: string | null }) {
  const qc = useQueryClient();
  ...
  const { data: watches } = useQuery({
    queryKey: ["watches", shopId, userId],
    enabled: !!userId,
    queryFn: async () => {
      const { data } = await supabase.from("appointment_watchers" as never)
        .select("id, watch_date, slot_time, notified_at")
        .eq("shop_id", shopId)
        .eq("user_id", userId as string)
        .order("watch_date", { ascending: true });
      return (data as { id: string; watch_date: string; slot_time: string | null; notified_at: string | null }[] | null) ?? [];
    },
  });
  ...
  const add = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("appointment_watchers" as never).insert({
        user_id: userId, shop_id: shopId, watch_date: date,
        slot_time: slot ? slot + ":00" : null,
      } as never);
      if (error) throw error;
    },
    onSuccess: () => { toast.success("Boş randevu çıkınca bildirim gönderilecek"); qc.invalidateQueries({ queryKey: ["watches", shopId, userId] }); },
    onError: (e: Error) => toast.error(e.message.includes("duplicate") ? "Bu tarih zaten izleniyor" : e.message),
  });

  const remove = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("appointment_watchers" as never).delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["watches", shopId, userId] }),
  });
  ...
  return (
    <section className="mt-6">
      <h2 className="mb-2 font-display text-lg tracking-wider">Boş Randevu Bildirimi</h2>
      <div className="rounded-xl border border-border bg-card p-3 space-y-2">
        <p className="text-xs text-muted-foreground">İstediğin tarih doluysa bekleme listesine ekle — iptal olduğunda haber verelim.</p>
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="text-[11px] text-muted-foreground">Tarih</label>
            <input type="date" value={date} min={new Date(Date.now()+86400000).toISOString().slice(0,10)}
              onChange={(e) => setDate(e.target.value)}
              className="w-full h-10 rounded-md border border-border bg-background px-2 text-sm" />
          </div>
          <div>
            <label className="text-[11px] text-muted-foreground">Saat (opsiyonel)</label>
            <input type="time" value={slot} onChange={(e) => setSlot(e.target.value)}
              className="w-full h-10 rounded-md border border-border bg-background px-2 text-sm" />
          </div>
        </div>
        <Button onClick={() => add.mutate()} disabled={add.isPending} className="w-full">İzlemeye Ekle</Button>
        {(watches ?? []).length > 0 && (
          <div className="pt-2 border-t border-border space-y-1.5">
            <p className="text-[11px] text-muted-foreground">İzlediklerim</p>
            {(watches ?? []).map((w) => (
              <div key={w.id} className="flex items-center justify-between rounded-md bg-muted px-2 py-1.5 text-xs">
                <span>
                  {new Date(w.watch_date).toLocaleDateString("tr-TR", { day: "numeric", month: "long" })}
                  {w.slot_time && ` · ${w.slot_time.slice(0,5)}`}
                  {w.notified_at && <span className="ml-2 text-emerald-500">✓ bildirildi</span>}
                </span>
                <button onClick={() => remove.mutate(w.id)} className="text-destructive text-xs">Sil</button>
              </div>
            ))}
          </div>
        )}
