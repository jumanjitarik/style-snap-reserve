...
const searchSchema = z.object({ shop: z.string().optional() });

export const Route = createFileRoute("/_authenticated/randevu-al")({
...
function BookPage() {
  ...
  const { shop: initialShop } = Route.useSearch();
  const [step, setStep] = useState<1|2|3|4|5>(initialShop ? 2 : 1);
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      if (!u.user) throw new Error("Giriş gerekli");
    ...
    onSuccess: () => { toast.success("Ödeme alındı, randevu onaylandı!"); navigate({ to: "/randevularim" }); },
  ...
  return (
    <AppShell>
      ...
      <div className="px-4 pb-4 space-y-4">
            ...
            <div className="space-y-2">
              {(shops ?? []).map((s) => (
                <button key={s.id} onClick={() => { setShopId(s.id); setStep(3); }}
                  className={cn("w-full text-left rounded-xl border p-3", shopId === s.id ? "border-primary" : "border-border bg-card")}>
        ...
        {step === 3 && (
          <>
            <button onClick={() => setStep(2)} className="text-xs text-primary">← Salon</button>
            <h2 className="font-display text-xl">Hizmet Seç</h2>
            <div className="space-y-2">
              {(services ?? []).map((s) => (
                <button key={s.id} onClick={() => { setServiceId(s.id); setStep(4); }}
                  className={cn("w-full text-left rounded-xl border p-3", serviceId === s.id ? "border-primary" : "border-border bg-card")}>
            ...
            {(staff ?? []).length > 0 && (
              <>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2">
                  <button onClick={() => setStaffId(null)} className={cn("rounded-full px-3 py-1.5 text-xs border whitespace-nowrap", !staffId ? "bg-primary text-primary-foreground border-primary" : "border-border")}>
                    Farketmez
                  </button>
                  {(staff ?? []).map((p) => (
                    <button key={p.id} onClick={() => setStaffId(p.id)} className={cn("rounded-full px-3 py-1.5 text-xs border whitespace-nowrap", staffId === p.id ? "bg-primary text-primary-foreground border-primary" : "border-border")}>
                      {p.name}
                    </button>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {SLOTS.map((s) => (
                <button key={s} onClick={() => setTime(s)}
                  className={cn("rounded-lg border py-2 text-sm", time === s ? "bg-primary text-primary-foreground border-primary" : "border-border bg-card")}>
                  {s}
                </button>
              ))}
            </div>
        ...
        {step === 5 && (
          <>
            ...
            <Button onClick={() => create.mutate()} disabled={create.isPending} className="w-full h-12 font-semibold">
              {create.isPending ? "İşleniyor..." : "Öde ve Onayla"}
            </Button>
            <p className="text-[10px] text-center text-muted-foreground">Stripe entegrasyonu için Pro plan gerekli — şu an demo kart ile çalışıyor.</p>
          </>
        )}
      </div>
    </AppShell>
